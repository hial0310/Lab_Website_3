...
app.get('/team_stats', function(req, res) {
  ...
  .then(data => {
    ...
    console.log("data[1].count =", data[1].count);
    console.log("data[1][0] =", data[1][0]);
    console.log("data[1][0].count =", data[1][0].count);
...
app.get('/player_info/select_player', function(req, res) {
  player = req.query.player_choice
  var query4 = 'select * from football_players WHERE id=$1;';
  var query5 = 'select * from football_players;';
  var query6 = 'select COUNT(home_score) as count from football_games;';
  db.task('get-everything', task => {
      return task.batch([
          task.any(query4,[player]),
          task.any(query5),
          task.any(query6)
      ]);
  })
  .then(data => {
    //console.log("data[1][0] =", data[0]);
  	res.render('pages/player_info',{
  			my_title: "Player info page",
  			result_4: data[1],
  			result_5: data[0][0],
  			result_6: data[2][0].count
  		})
  })
  .catch(error => {
